...
class LinUCB(Base):
    ...
    def __init__(
        ...
        _model = self._init_action_model(model=model, action_ids=action_ids)
    ...
    def _init_action_model(self, model, action_ids):
    ...
    def _linucb_score(self, context, model_id):
    ...
    def add_action(self, actions, model_id=None):
        ...
        _model = self._init_action_model(model, new_action_ids)
        if self.db_type == "rlite":
            self._model_storage.save_model(model_id=model_id, model=_model)
        else:
            self._model_storage.save_model(_model)
